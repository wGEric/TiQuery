...
var logSummary = function(name, failures, total){
  return "*********** " +  name + ": (" + (total-failures) + '/' + total + ')'
}
...
QUnit.testDone = function(name, failures, total) {
  if(failures){
    logger(failures, logSummary(name, failures, total));
  }
};
...
QUnit.moduleDone = function(name, failures, total) {
  if(failures){
    logger(failures, logSummary('MODULE ' + name, failures, total));
  }
};
...
QUnit.done = function(failures, total) {
  var msg = logSummary("TEST SUMMARY: ", failures, total)
  logger(failures, msg);
};
